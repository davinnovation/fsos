...
def _copy_object(
    ...
    fsm._get_db()[bucket_name][object_name] = {"path": f"{bucket_name}/{object_name}"}
    fsm._copy_file(
...
def _remove_object(bucket_name: str, object_name: str, ROOT_PATH: str) -> bool:
    del fsm._get_db()[bucket_name][object_name]
    fsm._remove_file(normpath(join(ROOT_PATH, bucket_name, object_name)))
    return True


def _update_object(
    bucket_name, object_name: str, object: bytes, ROOT_PATH: str
) -> bool:
    # TODO
    return True
...
def _list_object(bucket_name: str, ROOT_PATH: str) -> list:
    return list(fsm._get_db()[bucket_name].keys())
